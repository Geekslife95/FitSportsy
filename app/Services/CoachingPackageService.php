...
class CoachingPackageService{
    public function getAllCoachingPackagesById(int $coachingId){
        return CoachingPackage::select('id','package_name','package_price','session_days','session_start_time','session_end_time')->where('coach_id',$coachingId)->where('is_active',CoachingPackage::STATUS_ACTIVE)->paginate(50);
    }
